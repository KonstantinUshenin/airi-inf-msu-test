...
safe = re.sub(r"\b(user|token)=\S+", r"\1=***", text)
...
сохраняется, а значение заменяется. Граница слова `\b` перед группой обязательна:
без неё шаблон совпадёт и внутри чужого имени поля — `superuser=ivanov`
превратится в `superuser=***`, то есть маскируется не то поле, которое просили.

Оговорка: это учебная демонстрация `re.sub`, а не готовая санитизация логов. В
бою секреты не вырезают регуляркой постфактум — их не пишут в лог вообще, а если
уж чистят, то по списку известных полей и на стороне логгера.
...
Ниже — как та же запись выглядит на модуле `csv`. **Для сдачи H1 это не нужно**:
сам модуль разбирается позже в курсе, здесь он приведён только чтобы вы знали,
куда смотреть в реальной работе. Блок продолжает решение выше и использует уже
собранный список `rows`:
