...
```bash
head -c 1000000 /dev/urandom > model.ckpt
git add -f model.ckpt && git commit -m "chore: добавить веса"   # упс

echo "*.ckpt" >> .gitignore   # см. оговорку про порядок правил ниже
git add .gitignore && git commit -m "chore: игнорировать веса"

echo "новые веса" >> model.ckpt
git status --short            # M model.ckpt — файл всё ещё отслеживается

git rm --cached model.ckpt
git commit -m "chore: убрать веса из-под контроля версий"

git status --short            # пусто
ls model.ckpt                 # файл на диске остался
```
...
Осторожно с `echo >> .gitignore`: файл читается сверху вниз и побеждает
последнее подошедшее правило. Если ниже уже стоит исключение вроде
`!baseline.pt`, дописанная в конец маска `*.pt` молча его перекроет, и ранее
видимый файл исчезнет из `git status`. Такие правила вставляют **выше**
исключения, то есть правят файл руками, а не дописывают в конец.
...
Хук здесь положен в `.git/hooks` — самый короткий путь, и именно он показывает
проблему: каталог `.git` не версионируется, поэтому такой хук есть только у вас.
Версионируемый вариант — каталог `.githooks/` в репозитории плюс
`git config core.hooksPath .githooks` (раздел 19 микро-лекции); он требуется в
задаче H9.
