...
откладывают командой `git stash` (справочник, раздел 8): она прячет
незакоммиченные изменения в отдельную стопку и делает репозиторий чистым.""")
...
git config --global credential.helper cache   # или в памяти, на 15 минут
```

Разница существенная: `store` кладёт токен **открытым текстом** в
`~/.git-credentials` (защита — только права на файл), `cache` держит его в
памяти вспомогательного процесса и забывает через заданное время
(`--timeout=3600`). На своей виртуалке годится `store`, на общей машине —
