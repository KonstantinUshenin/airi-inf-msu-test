...
> **справочник** (разделы 8–14) — его не демонстрируют, он нужен при решении
...
> используется, поэтому разобран в справочнике — раздел 8.""")
...
(вернуть их можно через `git reflog`, раздел 9). Это не единственный способ
...
раздел 8), но **оставляет его на диске**.""")
...
**Разделы 8–14 на занятии не демонстрируются.** Здесь собрано то, что
понадобится при решении задач и в реальной работе, но что не помещается в
пятнадцать минут демонстрации: индекс, спасение потерянных коммитов, работа с
сервером, настройка доступа, большие файлы и историческая справка про
`checkout`. Читайте по мере необходимости.""")

md("""## 8. Индекс: коммит по частям
...
md("""## 9. Спасательный круг: `git reflog`
...
md("""## 10. Пустой коммит: `--allow-empty`
...
md("""## 14. Историческая справка: `checkout` против `switch` / `restore`
