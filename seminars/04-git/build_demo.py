...
code("""%%bash
rm -rf ~/seminar-04/git-demo        # чистый старт: ноутбук можно прогнать заново
...
2. **История не линейна.** Одновременно существуют ветки, коммиты приходят от
   ...
   работать без единого центра: хэш считается локально, и вероятность совпасть с
   чужим настолько мала, что на практике ею пренебрегают.""")
...
code("""%%bash
cd ~/seminar-04/git-demo || exit 1
sed -i 's/0\\.93/0.95/' train.py   # sed -i заменяет текст прямо в файле: 0.93 → 0.95
                                  # правим уже отслеживаемый файл: подняли метрику
...
коммит, файлы никуда не копируются, поэтому она стоит 41 байт (40 символов хэша
плюс перевод строки — увидим это в разделе 13) и заводить её под каждую задачу
нормально.
...
Сообщение у коммита слияния будет «Слить feature/plot» — не по Conventional
Commits из раздела 4, и это нормально: merge-коммиты под соглашение не
подгоняют, их сообщение обычно генерирует сам git.""")
...
git status                    # без --short: git прямым текстом пишет, что делать дальше
...
Где правило действует, задаёт **слэш в начале**. Без него (`__pycache__/`,
`*.pt`) правило работает на любой глубине — поэтому одной строки хватает на весь
проект. Слэш в начале прибивает правило к корню: `/__pycache__/` скроет только
каталог рядом с `.gitignore`, а такой же каталог внутри `src/` останется на
виду. Маска `**` нужна для промежуточных уровней — например, `data/**/*.csv`.
...
- **перезапустить проверку на сервере.** Автопроверки (CI — раздел 21)
  запускаются на новый коммит или push. Если джоб упал не по вашей вине — отвалилась сеть, кончилось
...
# awk здесь просто вытаскивает нужное поле из строки вывода
T=$(git cat-file -p "$H" | awk '/^tree /{print $2}')     # строка «tree <хэш>» → хэш
echo "--- tree: каталог проекта, имена файлов лежат здесь ---"
git cat-file -p "$T"

B=$(git cat-file -p "$T" | awk '$4 == "train.py" {print $3}')   # строка про train.py → её хэш
