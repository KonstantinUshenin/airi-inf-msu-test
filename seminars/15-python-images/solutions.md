...
def main():
    ...
    if after != before:
        print("множество значений маски изменилось")
        sys.exit(1)
...
Проверка сравнивает множества на равенство, а не на вложенность: при сильном
уменьшении редкий класс может пропасть целиком, и это тоже поломка разметки,
хотя новых значений не появилось.
