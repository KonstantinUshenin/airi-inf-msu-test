...
```bash
pip install pillow numpy tifffile pydicom matplotlib opencv-python
```
...
У GIF оговорка: само сжатие действительно без потерь, но перевод фотографии в палитру из 256 цветов теряет цвета необратимо. «Без потерь» здесь про алгоритм сжатия, а не про то, что фото доедет целым.
...
md("""
Вторая частая нормализация — привести данные к нулевому среднему и единичному разбросу. Именно её ждут предобученные модели, и именно поэтому в их описании публикуют `mean` и `std`.
""")

code("""
standardized = (normalized - normalized.mean()) / normalized.std()
print(round(float(standardized.mean()), 6), round(float(standardized.std()), 4))
""")
...
md("""
В теме про изображения смотреть только на числа мало — выведем результат глазами. `imshow` для RGB достаточно одной строки.
""")
