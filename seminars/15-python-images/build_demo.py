...
Поэтому семинар — про загрузку: какие бывают форматы, что каждый из них хранит и теряет, и как превратить файл в массив, с которым можно работать дальше.
""")
...
>
> Работаем **в домашнем каталоге** (`~/seminar-15/`), запуская Jupyter оттуда:
> относительные пути в ячейках рассчитывают на это. Каталог `/tmp` не
> используем — он вычищается при перезагрузке.
""")
...
question(
    ...
    "Порядок осей у них разный: `size` — это `(ширина, высота)`, а `shape` — `(высота, ширина, каналы)`. На квадратном изображении ошибка не видна, а на кадре 640×480 `size` даст `(640, 480)`, `shape` — `(480, 640, 3)`. Отсюда берутся перевёрнутые картинки и несовпадение размеров при ресайзе.",
)
...
question(
    ...
    "Для фотографий, где нужен общий вид, — обычно да, так устроено большинство открытых датасетов. Нельзя там, где важно точное значение пикселя: маски сегментации, шестнадцатибитные снимки, медицинские и научные данные. Здесь максимальное расхождение — 171 уровень из 255, то есть в отдельных точках цвет изменился радикально. Отдельная ловушка — пересохранение: каждый цикл «открыл — сохранил» добавляет новые потери.",
)
...
# convert("L") для режима I;16 не растягивает диапазон, а обрезает его.
...
code("""
# Кроп — обычный срез: сначала строки, потом столбцы. Копирования не происходит.
...
code("""
print("после NEAREST :", levels(mask.resize((64, 64), Image.NEAREST)))    # классы целы
print("после BILINEAR:", levels(mask.resize((64, 64), Image.BILINEAR)))   # классов стало 53
""")
...
Проявляется оно противно: на снимках природы модель почти не теряет в качестве
(зелень остаётся зеленью по яркости), а вот на медицинских изображениях с
псевдоцветом или на спутниковых каналах — рушится полностью. Ошибка при этом
молчаливая: ни исключения, ни предупреждения.
""")
