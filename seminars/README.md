...
Граница семинаров 7 и 8: передача файлов (`scp`, `rsync`) и туннели целиком
отнесены к 8, поэтому в 7 их нет — там сеть разбирается как диагностика по
слоям. По той же причине `curl` в 7 используется как измерительный прибор, а
работа с REST API и разбор JSON остаются семинару 11. Устаревшие `ifconfig` и
`netstat` в 7 только упоминаются: вместо них берём `ip` и `ss`.
