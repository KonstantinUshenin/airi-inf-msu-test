...
удостоверяющий центр, MITM, почему публичный сервис не оставляют на HTTP.
Сейчас демонстрация целиком локальная (свой `python3 -m http.server` и свой
HTTPS на `openssl s_server`) — так занятие не зависит от внешних сервисов.
