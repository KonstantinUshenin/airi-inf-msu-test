...
mkdir -p \
  "$assets_dir/B3" \
  "$assets_dir/M2/modules" \
  "$assets_dir/M3/project-source" \
  "$assets_dir/H3/bin-one" \
  "$assets_dir/H3/bin-two" \
  "$assets_dir/H5/broken-project/report"

cat > "$assets_dir/B3/course.env" <<'EOF'
COURSE_NAME='Linux and Python'
DATA_DIRECTORY='./course data'
EOF

cat > "$assets_dir/M2/modules/course_module.py" <<'EOF'
VALUE = "module from assets"
EOF

cat > "$assets_dir/M3/project-source/pyproject.toml" <<'EOF'
[project]
name = "restored-course-project"
version = "0.1.0"
requires-python = ">=3.12"
dependencies = ["requests==2.31.0"]
EOF

cat > "$assets_dir/H3/bin-one/course-info" <<'EOF'
#!/usr/bin/env bash
echo 'command from bin-one'
EOF
cat > "$assets_dir/H3/bin-two/course-info" <<'EOF'
#!/usr/bin/env bash
echo 'command from bin-two'
EOF
chmod +x "$assets_dir/H3/bin-one/course-info" "$assets_dir/H3/bin-two/course-info"

cat > "$assets_dir/H5/broken-project/pyproject.toml" <<'EOF'
[project]
name = "broken-project"
version = "0.1.0"
requires-python = ">=3.12"
...
EOF
echo '3.12' > "$assets_dir/H5/broken-project/.python-version"
cat > "$assets_dir/H5/broken-project/app.py" <<'EOF'
import requests

print(f"requests={requests.__version__}")
EOF

cat > "$assets_dir/M3/project-source/uv.lock" <<'UV_LOCK'
version = 1
revision = 3
requires-python = ">=3.12"
...
cp "$assets_dir/M3/project-source/uv.lock" "$assets_dir/H5/broken-project/uv.lock"

cat > "$assets_dir/README.md" <<'EOF'
# Входные данные

- `B3` — файл переменных окружения.
- `M2` — импортируемый модуль.
- `M3` — проект с `pyproject.toml` и согласованным `uv.lock`.
- `H3` — две одноимённые команды с разным результатом.
- `H5` — сломанный проект: lock-файл создан для другой версии зависимости и требует обновления.

Lock-файл записывается непосредственно генератором, поэтому подготовка данных
