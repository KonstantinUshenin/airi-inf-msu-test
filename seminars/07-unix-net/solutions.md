...
Оба вызова занимают около двух секунд: три пакета уходят с паузой в секунду, а
на неотвечающем адресе к этому добавляется ожидание последнего ответа (`-W 2`).
Точное число зависит от версии `ping` и настроек системы, поэтому его и меряем,
а не переписываем из методички. Важна качественная разница: в первом случае
ответы есть, во втором — ни одного.
...
| зависание и таймаут | 3–4. хост или порт | `ping -c 2 -W 2 ХОСТ`, затем правила фаервола | пакеты отбрасываются молча; какая из двух ступеней — по одному таймауту не видно |
