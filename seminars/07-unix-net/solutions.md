...
echo "=== то же самое, но наружу и по https ==="
curl -v -s -o /dev/null -m 10 https://example.com/ 2>&1 | grep -E '^\*|^>' | head -14
```

Разбор локального вывода по ступеням:
...
строками `Connected to` и `> GET` появляется блок рукопожатия: `TLS handshake`,
выбранная версия протокола и шифр, проверка сертификата. Именно он измеряется
как `time_appconnect` в задаче M4.

Что должно быть в предсказании для `example.com`: строка о разрешении имени
(`Host example.com:443 was resolved`, адреса IPv4 и IPv6), блок `TLS handshake`
с версией и шифром, строка `Server certificate:` с subject и issuer, и уже
после них `> GET / HTTP/2`. Обратное тоже полезно заметить: в локальном выводе
нет ни одной из этих строк — не потому, что что-то сломано, а потому, что этих
двух ступеней в запросе просто не было.
