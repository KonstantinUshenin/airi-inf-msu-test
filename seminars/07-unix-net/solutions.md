...
| Симптом | Ступень | Команда | Вывод «идём выше» | Вывод «идём вниз» |
|---|---|---|---|---|
| (а) `Name or service not known` | 2. имя | `getent hosts ИМЯ` | адрес получен — значит, дело было в опечатке или кэше | пусто: имя не разрешается, проверяем `/etc/hosts` и `/etc/resolv.conf` |
| (б) мгновенный `Connection refused` | 4. порт | `ss -tlnp \| grep ПОРТ` на сервере | порт слушается — отказ прислал фаервол, смотрим `ufw status` | пусто: сервис не запущен или занял другой порт |
| (в) зависание и таймаут | 3–4. хост или порт | `ping -c 2 -W 2 ХОСТ` | хост отвечает — значит, молча отбрасывают именно порт, идём в правила фаервола | молчит и `ping` — проблема ниже: адрес, маршрут, выключенная машина |
| (г) `SSL certificate problem` | 5. приложение | `openssl s_client -connect ХОСТ:443 -servername ИМЯ` | `Verify return code: 0` — сертификат в порядке, дело в клиенте или его наборе корневых УЦ | ошибка проверки: сертификат самоподписанный, просрочен или не на это имя |
| (д) соединение мгновенно, ответа нет 30 с | 5. приложение | `curl -w '%{time_connect} %{time_total}'`, затем логи сервиса | в логе запрос есть — приложение его получило и думает: смотрим нагрузку и зависимости | в логе пусто: запрос до приложения не дошёл, возвращаемся на ступень порта |
| (е) `504 Gateway Timeout` | 5. приложение | логи обратного прокси и логи приложения | прокси дождался, но приложение ответило ошибкой — чиним приложение | прокси не дождался: приложение висит или его таймаут меньше, чем время ответа |
