...
`vimdiff old new` открывает два файла рядом с подсветкой различий; переход между окнами — `Ctrl+W`, затем стрелка. Он интерактивный, поэтому в ноутбуке его не запустить, но ту же идею «два файла бок о бок» показывает `diff -y`:
""")
...
# Локаль задана явно: в C.UTF-8 кириллица считается символами, и числа расходятся.
# Под LC_ALL=C оба счётчика дали бы одно и то же — символом там считается байт.
...
- `-exec КОМАНДА {} \\;` — выполнить команду для каждого найденного файла: `{}` подставляет путь, `\\;` закрывает команду. Точка с запятой экранирована обратным слэшем, потому что иначе её съела бы оболочка — для неё `;` это разделитель команд, и до `find` он бы не дошёл;
