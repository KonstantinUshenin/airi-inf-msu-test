...
Именно потому, что `ed` читает команды из потока, его можно скормить скрипту — что и делаем ниже.
...
# Байты и символы совпадают только на латинице: кириллица в UTF-8 занимает по два байта.
wc -c -m assets/contacts.txt   # один и тот же файл: сначала байты, потом символы
""")
...
`tr SET1 SET2` заменяет символы по одному: `tr ',' '\\t'` меняет запятые на табуляции, `tr '[:upper:]' '[:lower:]'` приводит текст к нижнему регистру, `-d` удаляет символы, `-s` сжимает повторы подряд идущих одинаковых, `-c` берёт **дополнение** набора — то есть «все символы, кроме перечисленных». Комбинация `tr -cs '[:alpha:]' '\\n'` заменяет всё, кроме букв, на перевод строки: так текст разбивают на слова.
