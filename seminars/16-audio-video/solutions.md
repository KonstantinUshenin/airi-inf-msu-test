...
`0.70685`. Разница — след `PCM_16`: при записи сэмплы легли на сетку квантования,
и самый громкий из них округлился вверх, до `0.050018` вместо `0.05`. RMS от
такого округления практически не меняется, а знаменатель отношения подрос, — так
что числа в примере честно совпадают с выводом кода, а не с формулой.
...
Нарезка на окна одинаковой длины — стандартный способ подать запись переменной
