...
code("""
%%bash
# Напоминание из семинара 2, одной строкой: кто я, где я и на каком ядре.
echo "$(whoami) в $(pwd)"
uname -srmo
""")
...
`/proc` и `/sys` — псевдофайловые системы. На диске их нет: содержимое генерирует ядро в момент чтения. У файлов `/proc` из-за этого нулевой размер, хотя `cat` печатает текст. В `/sys` устроено иначе: там каждый файл — один атрибут, и ядро сообщает размер страницы памяти (4096), сколько бы байт ни лежало внутри. Общее у них одно: числу в `ls -l` верить нельзя, читать надо целиком.

- `/proc/cpuinfo`, `/proc/meminfo` — процессор и память;
- `/proc/loadavg` — средняя загрузка за 1, 5 и 15 минут. Это не проценты, а среднее число задач, которым нужен процессор: значение 4 означает «в среднем четыре задачи готовы считать». В Linux в него попадают и процессы, ждущие диск, поэтому высокий load average при простое процессора означает не перегрузку вычислениями, а упор в ввод-вывод;
...
code("""
%%bash
ls -l /proc/meminfo                       # 0: содержимое сочиняется в момент чтения
ls -l /sys/block/$(lsblk -dn -o NAME,TYPE | awk '$2 == "disk" {print $1; exit}')/size
""")
...
question(
    "Почему у файла блочного устройства вместо размера напечатаны два числа, например `259, 0`?",
...
md("""
`lsblk -f` добавляет к дереву тип файловой системы, метку и UUID — именно по UUID разделы прописывают в `/etc/fstab`, потому что имена вроде `/dev/sda` меняются от загрузки к загрузке.
""")

code("""
%%bash
lsblk -f -e 7 | head -n 4
""")
...
code("""
%%bash
# pciutils и usbutils есть не в каждом минимальном образе.
command -v lspci > /dev/null || echo 'нет lspci: sudo apt install pciutils'
command -v lsusb > /dev/null || echo 'нет lsusb: sudo apt install usbutils'
...
md("""
`-k` отвечает на вопрос «кто управляет устройством», `-v` — «какие ресурсы оно занимает»: адреса памяти, номер прерывания, диапазоны портов. Скорость шины PCIe (`LnkCap` и `LnkSta`) лежит глубже — её печатает `lspci -vv`, и только от root: обычному пользователю ядро расширенную область конфигурации не отдаёт.
""")
...
```
0, NVIDIA A100-SXM4-40GB, 40960, 39012
1, NVIDIA A100-SXM4-40GB, 40960, 1024
```

Свободную память считают как разницу `memory.total` и `memory.used`. Без `nounits` в каждом поле останется `MiB`, и его пришлось бы вырезать вручную.

Обычный вывод, который вы увидите на сервере, выглядит так — это та же информация, но свёрстанная для человека:
...
| N/A   38C    P0    62W / 400W |  39012MiB / 40960MiB |     98%      Default |
+-------------------------------+----------------------+----------------------+
```

Смотрят на две колонки: `Memory-Usage` — влезет ли ещё один запуск, и `GPU-Util` — занята ли карта делом. Ниже в той же таблице идёт список процессов, которые её держат.
""")
...
В следующей ячейке `print` вызывается ровно один раз, до `fork`. Посмотрите, сколько строк окажется в выводе.
""")

code("""
%%bash
# print вызывается один раз. Посчитайте строки в выводе, прежде чем читать вопрос ниже.
python3 -c "import os; print('до fork'); os.fork() and os.wait()"
""")

md("""
Детей может быть сколько угодно: каждый `fork` добавляет ещё одну копию, а родитель забирает результаты по одному вызову `wait` на каждого. `os.waitstatus_to_exitcode` достаёт из статуса код возврата — именно так устроена задача H4.
""")
...
for number in range(3):
    if os.fork() == 0:
        os._exit(number)
codes = [os.waitstatus_to_exitcode(os.wait()[1]) for _ in range(3)]
print('коды детей:', sorted(codes))
"
""")
