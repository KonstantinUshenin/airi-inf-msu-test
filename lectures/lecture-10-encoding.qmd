...
subtitle: "Lecture 10 — Text data [EARLY DRAFT]"
...
**Для авторов. Нужно улучшить:** раздел про Unicode без картинок и примеров сейчас. Итоги раздела про регулярные выражения не дописаны. Нет вообще ничего про DFA / NFA / backtracking engines. Как code point переводится на русский?
...
* Structure
  ...
  * Ends with `m` (SGR — Select Graphic Rendition)
...
* Purpose
  ...
  * One code point → one storage unit
...
* **Language design**
  ...
  * `s[i]` means “i-th code unit”, not “i-th symbol”
...
* **Conclusion**
  ...
  * Higher-level “characters” require iteration
...
* В записи строки стоит различать байт, кодовую единицу, кодовую точку и графему.
* Существуют кодировки с постоянной (код Бодо, latin-1, ASCII, UTF-32) и переменной длиной на одну кодовую точку (UTF-16, UTF-32).
* Для кодировки ASCII нижняя часть универсальна (до 127) и содержит английский алфавит, знаки препинания и управляющие команды.
* Верхняя часть определяется региональным стандартом. Это позволяет локализовать интерфейс пользователя. Такой подход сильно ограничиваем возможности интернационализации и смешивания языков.
* Unicode (UTF-8, UTF-16, UTF-32) позволяют задать почти все существующие национальные символы, знаки, смайлы и другие символы. UTF-8 использует от 1 до 4 символов на кодовую точку. Это вызывает проблемы с технической реализацией строк в языках программирования.
* Специальные символы и ESC-последовательности позволяют создавать консольные приложения с подсветкой синтаксиса и псевдоокнами. Такой тип приложений всё ещё распространён.
...
* Строки часто являются ресурсами программы. Хранятся в .rodata части ELF-файла
* Реализация строк в C была настолько плохой, что многие современные технологии родились в попытке это исправить
* С-строки заканчиваются нулём, Pascal-строки явно содержат длину строки
* В Python все строки хранятся в UCS-1/2/4 формате, независимо от оригинальной кодировки. У этих кодировок фиксированный размер точки кодировая, что даёт доступ к n-й точке кодирования за O(1).
* В Java, JavaScript, Rust, C# используется кодировки с переменной длиной UTF-8/16, поэтому доступ к n-й точке кодирования за O(n).
* Языки содержат инструменты для быстрого конкатенирования строк, это аналоги StringBuilder из Java, или .join([]) как в Python
* В большинстве языков строки неизменяемые структуры или объекты. Они явно содержат запись своей длины, хеш. Строки могут интернироваться.
...
* glob (wildcards) -- механизм для работы со списками файлов. Regular Expression -- механизм для работы с текстовыми данными. Оба механизма поддерживаются в консоли и большинстве современных языков программирования.
* **Python `re`: стандартный модуль для работы с регулярными выражениями. Функции: `search`, `match`, `findall`, `sub`, `split`
* **sed** — потоковый редактор, **awk** — язык обработки данных с regexp для условий
